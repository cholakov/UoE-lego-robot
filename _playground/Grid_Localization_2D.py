...
def localize(arena,measurements,motions,sensor_right,move_right):
    ...
    for step in range(len(measurements)):
        ...
        for i in range(len(p)): # loop each row
            xx=[]
            for j in range(len(p[0])): # shift column
        ...
        for i in range(len(p)):
            for j in range(len(p[0])):                
                match = (measurements[step] == arena[i][j])
...
# Note: numerical accuracy matters, if you dont normalized after move, then p value is so small that it will
